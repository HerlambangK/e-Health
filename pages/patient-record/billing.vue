<template>
  <div class="md:ml-72 pb-10 flex-grow items-center justify-center bg-white rounded-md shadow-md px-4 border mx-11">
    <h2 className="mr-2 my-4 text-2xl font-semibold tracking-tight text-center">
      Biling Pembayaran
    </h2>

    <div class="grid md:grid-cols-4 grid-cols-2 gap-2 overflow-x-scroll">
      <a href="#"
        class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="flex flex-row justify-between">
          <h5 class="mb-2 text-md md:text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            Kontrol 1
          </h5>
          <UBadge variant="outline" class="my-1">BPJS</UBadge>
        </div>
        <UBadge variant="soft" class="my-1">12 Januari 2023</UBadge>
        <p class="my-2 font-normal text-gray-700 dark:text-gray-400">
          Keluhan anda adalah ini dan itu sangat bagus, silahkan melakukan bayar
        </p>
        <UButton>Bayar</UButton>
      </a>
      <a href="#"
        class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="flex flex-row justify-between">
          <h5 class="mb-2 text-md md:text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            Kontrol 1
          </h5>
          <UBadge variant="outline" class="my-1">BPJS</UBadge>
        </div>
        <UBadge variant="soft" class="my-1">12 Januari 2023</UBadge>
        <p class="my-2 font-normal text-gray-700 dark:text-gray-400">
          Keluhan anda adalah ini dan itu sangat bagus, silahkan melakukan bayar
        </p>
        <UButton>Bayar</UButton>
      </a>
      <a href="#"
        class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="flex flex-row justify-between">
          <h5 class="mb-2 text-md md:text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            Kontrol 1
          </h5>
          <UBadge variant="outline" class="my-1">BPJS</UBadge>
        </div>
        <UBadge variant="soft" class="my-1">12 Januari 2023</UBadge>
        <p class="my-2 font-normal text-gray-700 dark:text-gray-400">
          Keluhan anda adalah ini dan itu sangat bagus, silahkan melakukan bayar
        </p>
        <UButton>Bayar</UButton>
      </a>
      <a href="#"
        class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="flex flex-row justify-between">
          <h5 class="mb-2 text-md md:text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            Kontrol 1
          </h5>
          <UBadge variant="outline" class="my-1">BPJS</UBadge>
        </div>
        <UBadge variant="soft" class="my-1">12 Januari 2023</UBadge>
        <p class="my-2 font-normal text-gray-700 dark:text-gray-400">
          Keluhan anda adalah ini dan itu sangat bagus, silahkan melakukan bayar
        </p>
        <UButton>Bayar</UButton>
      </a>
      <a href="#"
        class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
        <div class="flex flex-row justify-between">
          <h5 class="mb-2 text-md md:text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            Kontrol 1
          </h5>
          <UBadge variant="outline" class="my-1">BPJS</UBadge>
        </div>
        <UBadge variant="soft" class="my-1">12 Januari 2023</UBadge>
        <p class="my-2 font-normal text-gray-700 dark:text-gray-400">
          Keluhan anda adalah ini dan itu sangat bagus, silahkan melakukan bayar
        </p>
        <UButton>Bayar</UButton>
      </a>
    </div>

    <!-- <NButton>Default</NButton> -->
    <!-- <n-button>naive-ui</n-button> -->
  </div>
  <div class="container mx-auto p-8 md:ml-64 flex-grow">
    <div class="grid grid-cols-4 gap-4 overflow-x-scroll">
      <!-- Card 1 -->
      <div class="bg-white rounded shadow p-4">
        <!-- Konten Card -->
        <h2 class="text-lg font-semibold mb-2">Card 1</h2>
        <p>Isi dari card ini.</p>
      </div>

      <!-- Card 2 -->
      <div class="bg-white rounded shadow p-4">
        <!-- Konten Card -->
        <h2 class="text-lg font-semibold mb-2">Card 2</h2>
        <p>Isi dari card ini.</p>
      </div>

      <!-- Card 3 -->
      <div class="bg-white rounded shadow p-4">
        <!-- Konten Card -->
        <h2 class="text-lg font-semibold mb-2">Card 3</h2>
        <p>Isi dari card ini.</p>
      </div>

      <!-- Card 4 -->
      <div class="bg-white rounded shadow p-4">
        <!-- Konten Card -->
        <h2 class="text-lg font-semibold mb-2">Card 4</h2>
        <p>Isi dari card ini.</p>
      </div>
      <!-- Card 4 -->
      <div class="bg-white rounded shadow p-4">
        <!-- Konten Card -->
        <h2 class="text-lg font-semibold mb-2">Card 4</h2>
        <p>Isi dari card ini.</p>
      </div>
      <!-- Card 4 -->
      <div class="bg-white rounded shadow p-4">
        <!-- Konten Card -->
        <h2 class="text-lg font-semibold mb-2">Card 4</h2>
        <p>Isi dari card ini.</p>
      </div>

      <!-- Tambahkan lebih banyak card sesuai kebutuhan -->
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style></style>